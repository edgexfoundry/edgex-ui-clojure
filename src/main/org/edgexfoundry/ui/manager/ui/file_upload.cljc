...
;;; Original code https://github.com/awkay/file-upload-demo

(ns org.edgexfoundry.ui.manager.ui.file-upload
  (:require
...
(s/def ::file (s/keys
                ...
                :opt [::progress ::status ::abort-id ::comp-id]))
    ...
    (dom/div {:style {:padding "1px"}}
      (dom/div nil
        (dom/div
          ...
          (cond
            (= :network-error status) (dom/div nil "Upload Failed (network error).")
            (= :too-large status) (dom/div nil "Failed. Too large."))
          (dom/i :.glyphicon:.fa.fa-close {:onClick (fn []
                                                      (when onCancel
